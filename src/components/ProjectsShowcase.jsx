...
function ProjectCard({ featured = false, project, priority = false }) {
  ...
  return (
    <article className='group h-full overflow-hidden rounded-xl border border-border bg-card transition duration-300 hover:-translate-y-0.5 hover:border-foreground/30'>
      <Link
      ...
      >
        ...
        >
          <ProjectImage
            ...
            title={project.title}
          />
          <div className='absolute inset-x-0 bottom-0 bg-linear-to-t from-background via-background/80 to-transparent p-4'>
            <div className='-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 sm:flex-wrap sm:overflow-visible sm:pb-0'>
              <span className='rounded-full border border-border bg-background/90 px-3 py-1 text-xs font-semibold text-foreground'>
                {getCategoryName(project)}
              </span>
              {project.lifecycle && (
                <span className='rounded-full border border-border bg-background/90 px-3 py-1 text-xs font-semibold capitalize text-muted-foreground'>
                  {project.lifecycle}
                </span>
              )}
            </div>
